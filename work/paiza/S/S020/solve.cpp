...
int solve(){
  rep (i, 101) rep (j, 101) rep (k, 101) dp[i][j][k]=INFLL;

  ll n;cin>>n;
  ll w[n]; rep(i, n){ cin>>w[i]; }
  ...
  rep(i, n){
    ...
    rep(j, m){
      rep(r, 101){
        if (dp[i][j][r]==INFLL) continue;
        if (r-w[i]>=0){
          dp[i+1][j][r-w[i]] = dp[i][j][r]; //まだいれる。
        }
        else{
          mincost = min(mincost, dp[i][j][r]);
        }
      }
    }
    rep(j, m){
      if (a[j]-w[i]>=0){
        dp[i+1][j][a[j]-w[i]] = min(mincost + c[j], dp[i+1][j][a[j]-w[i]]);
      }
    }
  }
  ll res = INFLL;
  rep(j, m) rep(r, 101){
    res = min(dp[n][j][r], res);
  }

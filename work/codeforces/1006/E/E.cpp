...
vector<vector<int>> memo(200000);
int p[200000];
vector<vi> dc;

vector<int> getmemo(int m){
  if (memo[m].size()>0) return memo[m];
  vector<int> y;
  for (auto x: dc[m]){
    y.insert(y.end(), all(getmemo(x)));
  }
  memo[m] = y;
  return y;
}
...
int solve(){
  ...
  rep(i, q){
    cin>>u>>k;
    vector<int> x = getmemo[u];
    if (x.size()<k) cout << -1 << endl;
    if (x.size()>=k) cout << x[k] << endl;
  }

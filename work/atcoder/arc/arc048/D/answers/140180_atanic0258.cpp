...
signed main() {
	...
	VAR(int, n);
	VEC_ROW(int, n, r, h);
	std::vector<std::vector<int>> c(3, std::vector<int>(100005, 0));
	REP(i, n) {
		++c[h[i] - 1][r[i]];
	}
	std::vector<int> cc(100005);
	REP(i, 100001) {
		cc[i] += c[0][i] + c[1][i] + c[2][i];
		cc[i + 1] += cc[i];
	}
	REP(i, n) {
		int w = 0, l = 0, d = 0;
		w = cc[r[i] - 1] + c[h[i] % 3][r[i]];
		l = cc[100000] - cc[r[i]] + c[(h[i] + 1) % 3][r[i]];
		d = c[h[i] - 1][r[i]] - 1;
		OUT(w)SP OUT(l)SP OUT(d)BR;
	}
	return 0;
}
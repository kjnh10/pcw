...
struct SGT
{
	...
	{
		seg[t].l=l;seg[t].r=r;seg[t].mx=-inf;seg[t].delta=0;
		if(l==r)return;
		int mid=(l+r)>>1;
...
int main()
{
	...
	for(int i=1;i<=m;i++)
	{
		...
		{
			int id=st[i][j];
			m1.add(l[id]+1,r[id]-1,1);
		}
		ans=max(ans,m1.query(i,m)-i+1);
	}
	printendl(ans-m);
	return 0;
}
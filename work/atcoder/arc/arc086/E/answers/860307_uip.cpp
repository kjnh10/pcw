...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	function<void(int,int)> dfs=[&](int v,int d){
		...
		for(int w:g[v]){
			dfs(w,d+1);
			if(MX<int>(mx,zeros[w].size())) ind=w;
		}
		swap(zeros[v],zeros[ind]);
		swap(ones[v],ones[ind]);
		int m=0;
		for(int w:g[v])if(w!=ind) MX<int>(m,zeros[w].size());
		rep(i,m){
			mint zero=zeros[v][mx-1-i];
			mint sum=ones[v][mx-1-i]/zeros[v][mx-1-i];
			mint al=zeros[v][mx-1-i]+ones[v][mx-1-i];
			for(int w:g[v])if(w!=ind){
					mint z=1,o=0;
					int sz=zeros[w].size();
					if(sz>i){
						z=zeros[w][sz-1-i];
			...
			ones[v][mx-1-i]=zero*sum;
			zeros[v][mx-1-i]=al-ones[v][mx-1-i];
		}
		ones[v].pb(1);
		zeros[v].pb(1);
		out(v,ones[v],zeros[v],1);
	};
	dfs(0,0);
	mint re=0;
	rep(i,ones[0].size()) re+=ones[0][i]*modpow(2,n-cnt[ones[0].size()-1-i]);
	cout<<re<<endl;
  return 0;
}

...
inline int dot(const PAIR& a, const PAIR& b) {
	return a.first * b.first + a.second * b.second;
}

inline int cross(const PAIR& a, const PAIR& b) {
...
signed main() {
	...
	int right = 0, over = 0;
	REP(i, n) {
		std::vector<PAIR> p(n - 1);
		int tp = 0;
		REP(j, n) {
			if (i == j) continue;
			p[tp++] = PAIR(x[j] - x[i], y[j] - y[i]);
		}
		if (x[i] == 1 && y[i] == 2) {
			++right;
			++right;
		...
		std::sort(ALL(p), [](PAIR l, PAIR r) {return std::atan2(l.second, l.first) < std::atan2(r.second, r.first); });
		p.insert(p.end(), ALL(p));
		int r = 1, r2 = 1;
		REP(l, n - 1) {
			CHMAX(r, l + 1);
			while (cross(p[l], p[r]) > 0 && dot(p[l], p[r]) > 0) {
				++r;
			}
			if (cross(p[l], p[r]) > 0 && dot(p[l], p[r]) == 0) {
				++right;
				++r;
			}
			CHMAX(r2, l + 1);
			while (cross(p[l], p[r2]) > 0 && p[l] != p[r2]) {
				++r2;
			}
			over += r2 - r;
		}
	...
	int sum = n * (n - 1) * (n - 2) / 6;
	sum -= right + over;
	OUT(sum)SP OUT(right)SP OUT(over)BR;
	return 0;
}
...
int main() {
	...
	ll p[4] = { (int)1e9,(int)-1e9,(int)1e9,(int)-1e9 };
	rep(i, n)cmin(p[0], x[i].first + x[i].second), cmax(p[1], x[i].first + x[i].second), cmin(p[2], x[i].first - x[i].second), cmax(p[3], x[i].first - x[i].second);
	int e = max(p[1] - p[0], p[3] - p[2]);
	rep(i2, 2)rep(j2, 2)rep(i, 2)rep(j, 2) {
		pii X{ ((i2 ? p[0] + e / 2 : p[1] - e / 2) + (j2 ? p[2] + e / 2 : p[3] - e / 2)) / 2,((i2 ? p[0] + e / 2 : p[1] - e / 2) - (j2 ? p[2] + e / 2 : p[3] - e / 2)) / 2 };
		bool f = 1;
		ll d = (ll)abs(x[0].first - X.first) + abs(x[0].second - X.second);
		rep(k, n) {
			if (d != (ll)abs(x[k].first - X.first) + abs(x[k].second - X.second))f = 0;
		}
		if (f) {
